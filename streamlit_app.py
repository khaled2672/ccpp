...
# Initialize sliders state on first run
for feature, (low, high) in feature_bounds.items():
    key = f"slider_{feature}"
    if key not in st.session_state:
        st.session_state[key] = (low + high) / 2

st.sidebar.subheader("Input Parameters")
for feature, (low, high) in feature_bounds.items():
    key = f"slider_{feature}"
    st.session_state[key] = st.sidebar.slider(feature, low, high, st.session_state[key])
...
with col3:
    st.metric("Ensemble", f"{ensemble_pred:.2f} MW")
