...
def predict_power(features):
    ...
    ensemble_pred = models['best_weight'] * rf_pred + (1 - models['best_weight']) * xgb_pred
    return {
        'rf': rf_pred,
        'xgb': xgb_pred,
...
def map_columns(df):
    column_mapping = {
        ...
        "Actual Power": ["Actual Power", "Total Power", "Power Output", "Real Power", "Observed Power", "Measured Power", "TP", "PE"]
    }
    ...
    if len(mapped_columns) < 4:
        missing_cols = [col for col in column_mapping if col not in mapped_columns]
...
with col1:
    st.metric("Optimal Power Prediction", f"{predictions['ensemble']:.2f} MW", help="Combined prediction using both models")

    if show_individual:
        st.subheader("Individual Model Predictions")
        col1a, col1b = st.columns(2)
        with col1a:
            st.metric("Random Forest", f"{predictions['rf']:.2f} MW", delta=f"{predictions['rf'] - predictions['ensemble']:.2f} vs ensemble")
        with col1b:
            st.metric("XGBoost", f"{predictions['xgb']:.2f} MW", delta=f"{predictions['xgb'] - predictions['ensemble']:.2f} vs ensemble")
    ...
    with tab1:
        ...
        ax1.pie([models['best_weight'], 1 - models['best_weight']], labels=['Random Forest', 'XGBoost'], autopct='%1.1f%%')
        st.pyplot(fig1)
    with tab2:
        fig2, ax2 = plt.subplots()
        pd.Series(models['rf_model'].feature_importances_, index=['Temp', 'Humidity', 'Pressure', 'Vacuum']).plot(kind='barh', ax=ax2)
        st.pyplot(fig2)
...
with col2:
    ...
    }).plot(kind='bar', ax=ax3, color=["#1f77b4", "#ff7f0e", "#2ca02c"])
    ax3.set_ylabel("Power Output (MW)")
    plt.xticks(rotation=0)
    st.pyplot(fig3)

    st.subheader("Feature Correlations")
    corr = pd.DataFrame(np.random.randn(100, 5), columns=['Temp', 'Humidity', 'Pressure', 'Vacuum', 'Power']).corr()
...
if uploaded_file is not None:
    ...
    if df_processed is not None:
        ...
        actual_col_found = 'Actual Power' in df_processed.columns
        ...
        if auto_optimize:
            if actual_col_found:
                ...
                st.info(f"Available columns: {', '.join(df_processed.columns)}")
